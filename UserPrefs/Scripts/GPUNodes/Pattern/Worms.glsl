...
// Calcula las coordenadas 3D aplicando escala, escala por ejes y Domain Warping
vec3 getCoords(vec2 uv_val) {
    float fx = (Scale_X * 10.0) * Scale;
    float fy = (Scale_Y * 10.0) * Scale;
    float fz = (Scale_Z * 10.0) * Scale;
    
    vec3 p3 = vec3(uv_val.x * fx, uv_val.y * fy, uv_val.x * fz);
    
    // Domain Warping / Distorsión del espacio
...
// Evalúa y retorna dinámicamente el valor del patrón SELECCIONADO (Lógica plana sin #elif)
float getSample(vec2 uv_val) {
    vec3 p3 = getCoords(uv_val);
    float t_var = max(0.001, Luminance * 0.6);
    vec4 s1 = variations(simplex3d(p3 * 16.0), t_var);
    
    // Inicialización por defecto (Worms)
    float result = processNoiseValue(s1.x); 
...
void main() {
    // --- EVALUACIÓN DE LAS VARIACIONES COMO VARIABLES LOCALES ---
    vec3 p3 = getCoords(UVCoord);
    float t_var = max(0.001, Luminance * 0.6);
    vec4 s1 = variations(simplex3d(p3 * 16.0), t_var);
    vec4 s2 = simplex3d_fractal(p3 * 8.0 + 8.0, t_var);
    ...
    // Muestreo dinámico para el cálculo de normales basado en la selección activa del enum
